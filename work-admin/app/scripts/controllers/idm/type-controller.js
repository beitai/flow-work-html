...
(function() {
  'use strict';
// 分类的控制器
  angular.module('adminApp').controller('TypeController', function($scope,$stateParams,$q) {
    // $scope.groupService = $scope.IdmService($scope.restUrl.idmGroups);
    $scope.groupService = $scope.IdmService($scope.restUrl.idmTypes);
...
// 查询分类所有
    ...
    $scope.editGroup = function(id) {
      $scope.editModal({
        ...
        formData : {
          ...
          parentName : $scope.parentGroup.moduleType
        },
    ...
    $scope.tableOptions = {
      ...
      data : 'queryResult',
      colModels : [
        {name:'名称',index:'moduleType',sortable:true,width:'10%'},
        {name:'类型',index:'parentId',sortable:true,width:'7%',
          formatter:function(){
            return '<span ng-if="row.parentId==0">父级</span>'+
              '<span ng-if="row.parentId==1">子级</span>';
          }
        },
